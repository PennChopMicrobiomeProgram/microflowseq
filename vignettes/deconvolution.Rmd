...
```{r}
pb_postsep <- cbind(
  small = c(small.fraction = 18 / 21, large.fraction = 3 / 21),
  large = c(small.fraction =  5 / 13, large.fraction = 8 / 13))
pb_postsep
```
...
Solve for the correct relative abundance of balls and compare to the correct
answer. Success!

```{r}
apply(pb_sorted_relabund %*% t(pb_postsep), 2, normalize_sum)
